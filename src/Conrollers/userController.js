...
const createUser = async function (req, res) {
    try {
        ...
        if (!isValid(roleData)) return res.status(400).send({ status: false, message: "Role is mandatory in the request" })
        if (!["Author", "Admin", "Retail Users"].includes(roleData)) return res.status(400).send({ status: false, message: "Invalid role selected" });
        ...
        address = data.address
            ...
            const isValideStatus = function (Role) {
                return ["Author", "Admin", "Retail Users"].includes(Role)
            }
            if (Role) {
                if (!isValideStatus(Role))
                    return res.status(400)
                        .send({ status: false, message: "status Value should be [Author, Admin, Retail Users]" })
